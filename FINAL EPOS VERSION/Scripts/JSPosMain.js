...
function getTicketNo3() {
    // Replace this placeholder with your actual ticket retrieval logic
    const urlParams = new URLSearchParams(window.location.search);
    const id = urlParams.get("TicketID");
    alert('function getTicketNo().ln 170 id=' + id + 'count=' + count + ',gtotal=' + gtotal + ',getCustomerID=' + getCustomerID);
    if (id) {
...
function welcomeFunction() {
    alert("TicketNo=" + getTicketNo() + ", TicketID=" + getTicketID() + ", getCustomerID=" + getCustomerID() + ",getCustomerAccountNo=" + getCustomerAccountNo());
}
function getTicketNo() {
    // Get the div that contains the Literal control
    var ticketNoDiv = document.getElementById("TicketNo");

    // Get the text content
    var literalText = ticketNoDiv.textContent || ticketNoDiv.innerText;

    // Return or do something with it
    console.log("Literal Text:", literalText.trim()); // or return literalText;

    return literalText.trim();
};

function getTicketID() {
    var literal = document.getElementById("hiddenLiteral");
    if (literal) {
        var text = literal.textContent || literal.innerText;
        console.log("Literal Ticket ID:", text);
        return text.trim();
    } else {
        console.log("Literal_TicketID not found in DOM.");
        return null;
    }
};

function getCustomerID() {
    var customerIDElement = document.getElementById("customer_id_Selected");
    return customerIDElement ? customerIDElement.value : null;
};

function getCustomerAccountNo() {
    var accountNoElement = document.getElementById("customer_CustomerAccNo");
    return accountNoElement ? accountNoElement.value : null;
};
...
$(document).ready(function () {
    ...
    $("#paymentZZ").click(function () {
        ...
        var Ticketno = getTicketID();
        alert('#JSPosMain.paymentZZ.ln.242 ticketno=' + Ticketno + ', count=' + count + ',gtotal=' + gtotal);
    ...
    $("#submit-sale22").click(function () {
        var count = document.getElementById("count").innerHTML;
        var gtotal = document.getElementById("total-payable").innerHTML;
        var payby = document.getElementById("paid_by").value;
        var note = document.getElementById("note").value;
        var mTicketID = getTicketID();
        var OrderId = getTicketID();
        //if (mTicketID = null) { TicketID = 66 };
        alert('JPosMain.js.submit-sale ln 251 count=' + count + ',gtotal=' + gtotal + ',payby=' + payby + ',Note=' + note + ',mTicketID=' + mTicketID);
        //mTicketID = document.getElementById("orderid").value;
        alert('JPosMain.js.submit-sale ln 253 TicketID=' + mTicketID + ',OrderId=' + OrderId);
    ...
    function process_payment(OrderId, gtotal, payby, count, note) {
        //alert('hh')
        alert('JSPosMain.process_payment.ln:260 OrderId=' + OrderId + ', gtotal=' + gtotal + ', count=' + count + ', payby=' + payby + ', note=' + note);
        var count = document.getElementById("count").innerHTML;
        var total = document.getElementById("total-payable").innerHTML;
        var payment_note = document.getElementById("payment_note").innerHTML;
        var amount = document.getElementById("total").innerHTML;
        alert("count=" + count + ",total=" + total + ",payment_note=" + payment_note + ",amount=" + amount);
        $.ajax({
    ...
    $("#print_order").click(function (t) {
        ...
        var rv = ""
        var url = "/POS/TestInvoicePopup.html?customer=Mandy%20Colman&ticketID=66&discount=5.00&product0=Trousers&price0=4.95&quantity0=3&total0=14.85&product1=Evening%20Dress&price1=15.00&quantity1=1&total1=15.00&product2=Shirt&price2=2.60&quantity2=1&total2=2.60&DateReceived=02/12/2025%2000:00:01&DateReceived=27/12/2025%2000:00:01";

        if (count <= 1) {
            bootbox.alert(lang.please_add_product + '\n###JPOSMAIN.JS#print_order#ln320##count=' + count);
        } else {
            bootbox.alert("save order procedure begins" + '\n###JPOSMAIN.#print_order#ln322##count=' + count)
            checkCollectDate();
            rv = saveOrder(TicketID)
            rv = document.getElementById('responseStorage').value
            alert("JSPosMain.ln:326.print_order.saveOrder.rv=" + rv)
                // fetchInvoiceData(rv)s
            setTimeout(function () {
                $("#order-data").hide();
            }, 500);
            processInvoice(url);
    ...
    $("#print_order22").click(function (t) {
        ...
        var count = $("#count").html()
        if (count <= 1) {
            bootbox.alert(lang.please_add_product + 'ln:341##JPOSMAIN.JS##' + count);
        } else {
            bootbox.alert("save order procedure begins" + 'ln:343##JPOSMAIN.#print_order##count=' + count)
...
function saveOrder(TicketID) {
    alert("ln340.JSPosMain.saveOrder.TicketID=" + TicketID);
...
function processInvoice(url) {
    // Parse URL parameters from the given URL
    const params = new URL(url).searchParams;

    // Debugging alert to notify function execution (fixing typo from "alerrt" to "alert")
    alert("JSPosMain.processInvoice ln 440");

    // Extract general invoice details from URL parameters
    const customer = params.get("customer") || "N/A"; // Get customer name, default to "N/A" if missing
    const ticketID = params.get("ticketID") || "N/A"; // Get ticket ID, default to "N/A"
    const discount = parseFloat(params.get("discount")) || 0.00; // Convert discount to a float, default to 0.00
    const dateReceived = params.getAll("DateReceived").join(", "); // Handle multiple "DateReceived" values, joining them with commas
    const detePromised = params.get("DetePromised") || "N/A"; // Get promised date, default to "N/A"

    // Extract product details dynamically
    const products = [];
    let index = 0;

    // Loop through available product parameters, storing details in an array in HTML Basket table
    // product${index} dynamically constructs parameter names like product0, product1, product2, etc.
    while (params.has(`product${index}`)) {
        products.push({
            product: params.get(`product${index}`), // Get product name dynamically using `product${index}`
            price: parseFloat(params.get(`price${index}`)), // Convert price to a float
            quantity: parseInt(params.get(`quantity${index}`)), // Convert quantity to an integer
            total: parseFloat(params.get(`total${index}`)) // Convert total to a float
        });
        index++; // Increment index for the next product in the parameters
    }
    ...
    // Iterate through each product in the array and sum up the quantity and total price
    products.forEach((item) => {
        totalQty += item.quantity;
        grandTotal += item.total;
    });
    ...
    // Generate the HTML structure for the invoice dynamically
    const invoiceHTML = `
        <div style="padding: 20px; font-family: Arial, sans-serif; border: 1px solid #ccc; border-radius: 10px; max-width: 600px; margin: auto; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
            <h2 style="text-align: center;">Peaches and Clean Invoice</h2>
            <p><strong>Invoice #:</strong> ${ticketID}</p>
    ...
    // Insert the generated invoice into the page
    const invoiceContainer = document.getElementById("invoiceContainer");
    if (invoiceContainer) {
        invoiceContainer.innerHTML = invoiceHTML; // Display invoice inside the container
    } else {
        alert("Error: No container found to display the invoice."); // Handle missing container error
    }
}
