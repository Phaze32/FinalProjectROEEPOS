...
function checkEmailAlndPasswordExists(oEmail, oPassword) {
    // $.blockUI({ message: $('#domMessage') });
    var result = null;
    $.ajax({
        type: "GET",
        url: 'ajaxcheckemail.asp',
        async: false,
        data: "act=0&email=" + oEmail.value + "&pwd=" + oPassword.value,
        dataType: "html",
        success: function (data) {
            result = data.toLowerCase() === 'true';
        },
...
$(document).ready(function () {
    ...
    $('#print_order').click(function (e) {
        ...
        if (count <= 1) {
            ...
            if (Settings.remote_printing == 0) {nli
                $('#order-data').show();
    ...
    $(document).on('click', '.product', function (e) {
        ...
        $.ajax({
            type: "get",
            url: base_url+'pos/get_product.aspx?PULid='+code,
            dataType: "json",
            success: function (data) {
                if (data !== null) {
...
// To delete item from the basket
        var pwacc = false;
        $(document).on('click', '.posdel', function () {
            var row = $(this).closest('tr');
            var item_id = row.attr('data-item-id');
            alert('posdel!');
            var ticket_id = row.attr('Hidden_TicketID');
            //alert('Hidden_TicketID=' + ticket_id );
            if(protect_delete == 1) {
                var boxd = bootbox.dialog({
                    title: lang.enter_pin_code,
                    closeButton: true,
