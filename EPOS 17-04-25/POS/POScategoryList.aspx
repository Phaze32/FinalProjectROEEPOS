...
<script type="text/javascript">
    $(document).ready(function () {
        var table = $('#catData').DataTable({
            ajax: {
                url: '/handlers/CategoryHandler.ashx', // Endpoint URL
                type: 'POST',
                dataType: 'json', // Ensures the response is treated as JSON
                dataSrc: '', // JSON is flat, so this is correct
            ...
            columns: [
                ...
                { data: "DeparmentName", title: "Department Name", searchable: true, orderable: true },
                {
                    ...
                    render: function (data) {
                        return data ? data : 'N/A'; // Display 'N/A' if ShopID is null
                    }
                },
                {
                    // Adding the editable button
                    data: "Rec_id",
                    title: "Actions",
                    orderable: false,
                    searchable: false,
                    render: function (data) {
                        ...
                        <button type="button" class="btn btn-sm btn-primary edit-btn" 
                                onclick="openCatagoryModal(event)" data-catagoryid="${data}">
                            Edit
                        </button>`;
                    }
                }
...
<script>
    ...
    function openCatagoryModal(event) {
        const Rec_id = event.target.getAttribute('data-catagoryid');
        alert('openCatagoryModal.Rec_id ID:' + Rec_id);

        console.log('Edit button clicked for Rec_id:', Rec_id);
        alert('Edit button clicked for Rec_id:' + Rec_id)
        // Add your modal opening logic here
    }
    function openUserModal(event) {
        var userId = $(event.target).data('userid'); // Get the Rec_id from data attribute
        console.log('Edit button clicked for Rec_id:', userId);
        // Add your modal opening logic here
    }
</script>
...
<script src="../scripts/pos.min.js" type="text/javascript"></script>
        <!-- #include virtual = "/POS/FormCatagoryModal.html" -->
</body>
</html>